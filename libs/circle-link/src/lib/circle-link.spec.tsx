...
describe('CircleLink', () => {
  it('should render successfully', () => {
    const { baseElement } = renderWithTheme(<CircleLink />)
    expect(baseElement).toBeTruthy()
  })

  it('should render with a label', () => {
    renderWithTheme(<CircleLink label="test" />)
    expect(screen.getByText('test')).toBeInTheDocument()
  })

  it('should render with a label inside the icon', () => {
    renderWithTheme(<CircleLink label="test" iconContent="wadus" />)
    expect(screen.getByText('wadus')).toBeInTheDocument()
  })

  it('should render with a different size', () => {
    const { baseElement } = renderWithTheme(
      <CircleLink label="test" iconContent="wadus" iconSize={50} />
    )
